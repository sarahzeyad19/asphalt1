# -*- coding: utf-8 -*-
"""
RUT_20K MODELING WORKFLOW v3.2 — NEW DATA (LWT__SCB_CLEANED.xlsx, sheet RUT) + AFT + RBR
                               + BALANCED-MIX PHYSICS INTERACTIONS + GROUP-SAFE SPLIT
                               + STACKING + REPEATED-CV ROBUSTNESS + FULL DIAGNOSTICS
Author: Updated for Sarah Al-Jezawi

v3.2 UPGRADES (this version)
----------------------------
1. NEW DATA FILE: LWT__SCB_CLEANED.xlsx sheet "RUT" (1890 rows) with the physics-rich columns
   AFT_micron (asphalt film thickness), RBR_percent, Pbe_pct, Gse, Dust_Pbe_ratio,
   SurfaceArea_m2kg, full gradation Grad_*, additive columns and MixTemperature_F_clean.
2. BALANCED-MIX ENGINEERED INTERACTIONS (engineering interpretation, advisor list):
   PG_x_RBR (aged binder + high-temp grade), RBR_x_AFT (RAP stiffening vs binder film),
   DustPbe_x_AFT (mastic stiffness vs film), Abs_x_AFT (absorption eating the film),
   Va_to_VMA (compactness), CoarseAgg_pct & CoarseAgg_x_NMAS (aggregate skeleton),
   Traffic_x_PG (grade vs loading severity).
3. THREE NEW FEATURE SETS: AFT_RBR_Physics, AFT_RBR_Physics_PlusInteractions,
   BalancedMix_Categorical — run alongside the previous sets so the gain is measured.
4. GROUP-SAFE PROTOCOL: GROUP_SPLIT_BY_MIX=True — ALL rows are used and every replicate of a
   mix stays in one split/fold (no replicate leakage in split, training CV, RepeatedCV, Nested CV).
5. ENGINEERING INTERPRETATION: SHAP + PDP on the physics set expose the rutting-vs-cracking
   trade-off (variables that help rutting — high RBR, stiff binder, high dust/binder, thin film —
   are the same ones that can hurt SCB; see RBR_x_AFT and DustPbe_x_AFT panels).
...
GROUP_SPLIT_BY_MIX = True   # v3.2: ALL 1890 rows are used; grouping (not averaging) stops leakage
GROUP_COL = ID_COL
...
HOME = Path.home()
# Input data + outputs live here. Falls back to the real Downloads folder automatically.
DOWNLOADS = Path(r"C:\Users\H0012066\Downloads")
if not DOWNLOADS.exists():
    DOWNLOADS = HOME / "Downloads" if (HOME / "Downloads").exists() else Path.cwd()
# v3.2: the NEW combined cleaned file with AFT_micron + RBR_percent (+ gradation, Pbe, Gse,
# Dust/Pbe, surface area, additives, mix temperature). Rutting rows live in sheet "RUT".
RUT_FILENAME = "LWT__SCB_CLEANED.xlsx"
RUT_FILE = DOWNLOADS / RUT_FILENAME
RUT_FILE_FALLBACKS = [
    DOWNLOADS / "LWT__SCB_CLEANED (1).xlsx",
    DOWNLOADS / "LWT_ SCB_CLEANED.xlsx",
    DOWNLOADS / "Rutting_Cleaned_with_RBR.xlsx",   # old file still accepted as a fallback
]
SHEET_CANDIDATES = ["RUT", "LWT_Clean_Modeling", "Cleaned_With_RBR", "Sheet1", 0]

OUTPUT_FOLDER = DOWNLOADS / f"Rut20k_v3_2_{SPLIT_TAG}_AFT_RBR_outputs"
...
def resolve_file_path(path: Path, fallbacks: List[Path]) -> Path:
    ...
    try:
        ...
        candidates += list(script_dir.glob("*LWT*SCB_CLEANED*.xlsx"))
        candidates += list(script_dir.glob("*Rutting_Cleaned_with_RBR*.xlsx"))
        candidates += list((HOME / "Downloads").glob("*LWT*SCB_CLEANED*.xlsx")) if (HOME / "Downloads").exists() else []
    except Exception:
        pass
    candidates += [Path.cwd() / p.name for p in list(candidates)]
    candidates += list(Path.cwd().glob("*LWT*SCB_CLEANED*.xlsx"))
...
ALIASES = {
    ...
    # In the new cleaned file the sieve columns are Grad_No4 (4.75 mm) and Grad_No200 (0.075 mm)
...
NUMERIC_HINTS = [
    ...
    # v3.2 physics-rich variables from the new cleaned file
    "AFT_micron", "Pbe_pct", "Dust_Pbe_ratio", "Gse", "SurfaceArea_m2kg",
    "Additive_Rate_clean", "MixTemperature_F_clean",
    "Grad_3_4in", "Grad_1_2in", "Grad_3_8in", "Grad_No8", "Grad_No30", "Grad_No50",
    # v3.2 engineered interactions (balanced-mix physics)
    "RBR_x_AFT", "DustPbe_x_AFT", "Va_to_VMA", "CoarseAgg_pct", "CoarseAgg_x_NMAS",
    "Traffic_x_PG", "Abs_x_AFT",
]
CATEGORICAL_HINTS = ["MixType", "DesignLev", "RAP_Class", "Additive_Type_clean", "Has_Additive"]
...
DROP_ALWAYS = [
    ...
    "Aggregate_components_used", "IsLeft", "ADT", "RBR_band", "RBR_Band",
    "Flag_RBR_out_of_range", "Flag_missing_input", "MixTemperature_flag_invalid",
]
...
def create_engineered_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ---- v3.2 balanced-mix physics interactions (advisor list) ----
    # RBR_x_AFT: is RAP-binder stiffening balanced by enough binder film?
    if has("RBR_JMF_percent", "AFT_micron"):
        df["RBR_x_AFT"] = pd.to_numeric(df["RBR_JMF_percent"], errors="coerce") * pd.to_numeric(df["AFT_micron"], errors="coerce")
    # DustPbe_x_AFT: mastic stiffness versus binder film thickness.
    if has("Dust_Pbe_ratio", "AFT_micron"):
        df["DustPbe_x_AFT"] = pd.to_numeric(df["Dust_Pbe_ratio"], errors="coerce") * pd.to_numeric(df["AFT_micron"], errors="coerce")
    # Abs_x_AFT: does aggregate absorption eat the effective binder film?
    if has("Absorption", "AFT_micron"):
        df["Abs_x_AFT"] = pd.to_numeric(df["Absorption"], errors="coerce") * pd.to_numeric(df["AFT_micron"], errors="coerce")
    # Va_to_VMA: compactness / available void structure.
    if has("Va", "VMA"):
        df["Va_to_VMA"] = safe_divide(df["Va"], df["VMA"])
    # CoarseAgg_pct (retained above 4.75 mm) and its skeleton interaction with NMAS.
    if "Pass4_75mm" in df.columns:
        df["CoarseAgg_pct"] = 100.0 - pd.to_numeric(df["Pass4_75mm"], errors="coerce")
        if "NMAS (mm)" in df.columns:
            df["CoarseAgg_x_NMAS"] = df["CoarseAgg_pct"] * pd.to_numeric(df["NMAS (mm)"], errors="coerce")
    # Traffic_x_PG: does the binder grade match the loading severity?
    if has("ADT_DOTD_ord", "PG_HighTemp"):
        df["Traffic_x_PG"] = pd.to_numeric(df["ADT_DOTD_ord"], errors="coerce") * pd.to_numeric(df["PG_HighTemp"], errors="coerce")
...
# ---- v3.2: physics-rich sets built on the NEW file's AFT / RBR / effective-binder columns ----
AFT_RBR_PHYSICS = VOLUMETRICS_B_RBR_BOTH + [
    "AFT_micron", "Pbe_pct", "Dust_Pbe_ratio", "Gse", "SurfaceArea_m2kg",
    "Additive_Rate_clean", "MixTemperature_F_clean",
]
BALANCED_MIX_INTERACTIONS = [
    "PG_x_RBR",           # aged binder + high-temperature grade effect
    "RBR_x_AFT",          # RAP stiffening balanced by binder film thickness
    "DustPbe_x_AFT",      # mastic stiffness vs binder film
    "Abs_x_AFT",          # absorption eating the effective film
    "Va_to_VMA",          # compactness / void structure
    "CoarseAgg_pct", "CoarseAgg_x_NMAS",   # aggregate skeleton (load-carrying structure)
    "Traffic_x_PG",       # binder grade vs loading severity
]
AFT_RBR_PHYSICS_PLUS = AFT_RBR_PHYSICS + BALANCED_MIX_INTERACTIONS
BALANCED_MIX_CATEGORICAL = AFT_RBR_PHYSICS_PLUS + ["MixType", "DesignLev", "Additive_Type_clean", "Has_Additive"]
...
FEATURE_SETS = {
    ...
    "AFT_RBR_Physics": AFT_RBR_PHYSICS,
    "AFT_RBR_Physics_PlusInteractions": AFT_RBR_PHYSICS_PLUS,
    "BalancedMix_Categorical": BALANCED_MIX_CATEGORICAL,
}
...
FEATURE_SETS_TO_RUN = [
    ...
    # v3.2 physics sets — usually the strongest with the new file
    "AFT_RBR_Physics",
    "AFT_RBR_Physics_PlusInteractions",
    "BalancedMix_Categorical",
]
if QUICK_SMOKE_TEST:
    FEATURE_SETS_TO_RUN = ["VolumetricsB_NoADT_RBR_Both", "AFT_RBR_Physics_PlusInteractions"]
...
def run_pdp(estimator, X, top_features, out_dir) -> List[Dict[str, Any]]:
    ...
    pair_candidates = [
        # v3.2 balanced-mix panels first: RAP stiffening vs binder film, mastic vs film
        ("RBR_JMF_fraction", "AFT_micron"), ("Dust_Pbe_ratio", "AFT_micron"),
        ("RBR_JMF_fraction", "PG_HighTemp"), ("CoarseAgg_pct", "NMAS (mm)"),
        ("RBR_JMF_fraction", "Absorption"),
