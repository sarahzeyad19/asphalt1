# -*- coding: utf-8 -*-
"""
RUT_20K MODELING WORKFLOW v3 — 80/20 SPLIT (TARGET INSIDE THE 80% TRAINING DATA)
                               + TRUE RBR + HIGH REGULARIZATION
...
WHAT CHANGED vs THE PREVIOUS 70/10/20 SCRIPT (and WHY)
------------------------------------------------------
This version keeps the whole modelling pipeline but changes the data split to the requested
80 / 20 protocol:

1.  SPLIT CHANGED TO 80 / 20 (train / locked-test). NO separate validation holdout.
    - Target-bin–stratified, reproducible random split (fixed RANDOM_STATE).
    - The 20% test set is split off FIRST and is NEVER touched during tuning, feature
      selection, or model selection. It is scored exactly once, at the very end, for the
      FINAL EVALUATION.
    - The full 80% is the TRAINING data. Supervised learning: each training row goes into
      the model as input features X TOGETHER WITH its measured Rut_20k target y — the target
      is the label the model learns to reproduce (it guides the loss and the weight updates).
      The 20% test rows are fed to the model WITHOUT their target; the hidden measured values
      are only used afterwards to score how well the predictions match.
    - Because there is no 10% validation slice, model/feature selection now uses the honest
      OUT-OF-FOLD (OOF) 5-fold cross-validation predictions computed INSIDE the 80% training
      data (every row is predicted by a model that never saw it). All "Validation_*" columns
      in the outputs therefore report these OOF CV scores.
...
def main():
    t0 = time.time()
    print("=" * 100)
    print(f"RUT_20K WORKFLOW v3 — {SPLIT_TAG.replace('_', '/')} + TRUE RBR + HIGH REG + STACKING + FULL DIAGNOSTICS")
    print("=" * 100)
    ...
    print("\n" + "=" * 100)
    print(f"FINISHED RUT_20K v3 {SPLIT_TAG.replace('_', '/')} WORKFLOW — {mode_label}")
    print("=" * 100)
