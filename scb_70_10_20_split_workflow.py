...
def load_data() -> Tuple[pd.DataFrame, pd.Series, Path]:
    ...
    # the physical modelling columns + target, and (when GROUP_SPLIT_BY_MIX) assign every such
    # identical mix a SHARED group id so any residual duplicates cannot span a split/fold.
    n_content = 0
    if DEDUP_BY_FEATURE_SIGNATURE:
        ...
        if len(sig_cols) >= 5:
            ...
            # give identical mixes a shared group id so grouping treats them as ONE mix
            ...
            if ID_COL in df.columns:
                # unify the mix id to the content group so no identical mix can span a split
