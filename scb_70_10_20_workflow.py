...
# =============================================================================
# RESTRICTED-RANGE FILTER + CALIBRATION + IMPORTANCE-BASED FEATURE SELECTION
# (requested: for SCB run ONLY on Jc < 1 — do not use data >= 1; adjust the final numbers with a
#  calibration equation; select the highest-impact features; keep the STRATIFIED split.)
# =============================================================================
# HARD target-range filter applied in load_data BEFORE the split. SCB request: keep only Jc < 1,
# DROP every row with Jc >= 1. The model is then an honestly-labelled "valid for Jc < 1" model.
RESTRICT_TARGET_RANGE = True         # SCB: drop rows with Jc >= 1
TARGET_RANGE_MIN = None              # inclusive lower bound (None = -inf)
TARGET_RANGE_MAX = 1.0               # upper bound
TARGET_RANGE_MAX_INCLUSIVE = False   # False -> "Jc < 1" excludes exactly 1.0

# REPORT (do NOT drop) a reliable band beside the full range. Not needed for SCB (the hard filter
# already restricts to Jc < 1), so leave it off.
REPORT_INRANGE_BAND = None           # (min, max) or None

# Compression-calibration equation. Fit  Measured = a * Predicted + b  on the DEV predictions and
# apply it to every split so Predicted approx Measured (fixes slope<1 compression -> lower
# RMSE / MAE / bias). HONEST NOTE: the reported R2 is the coefficient of determination
# (r2_score = 1 - SSres/SStot), which is NOT invariant to rescaling the prediction -- so correcting
# the compression typically RAISES the test R2 too. Legitimate: a and b are learned on DEV only and
# applied once to the locked test (part of the model, not tuning on the test).
APPLY_CALIBRATION = True

# Importance-based feature selection: rank pooled candidate features by RandomForest importance on
# the DEV rows only (no locked-test leakage) and register a reduced "TopImpact_Selected" set of the
# K most impactful features, run through the normal pipeline alongside the other feature sets.
...
def load_data() -> Tuple[pd.DataFrame, pd.Series, Path]:
    ...
    # ---- HARD restricted-range filter (SCB request: keep only Jc < 1, drop Jc >= 1) ----
