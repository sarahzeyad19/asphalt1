...
# ---- SPLIT REQUESTED BY USER: 70 / 10 / 20 ----
TRAIN_SIZE = 0.70
...
OUTPUT_FOLDER = DOWNLOADS / "Rut20k_v2_70_10_20_RBR_outputs"
...
VOLUMETRICS_B_RBR_BOTH = VOLUMETRICS_B_NOADT + ["RBR_JMF_fraction", "RBR_JMF_percent"]
...
FULL_EXPANDED_CLEAN = RUT_BASE_NOADT + [
    ...
    "CAA", "Gsb", "Va", "Gmm", "Gmb", "RBR_JMF_fraction", "RBR_JMF_percent",
]
...
def define_models(feature_set: str) -> Dict[str, Dict[str, Any]]:
    ...
    if HAS_XGBOOST:
        models["XGBoost"] = {
            "estimator": make_xgb(True),
            "n_iter": N_ITER_XGB,
            "params": {
                "model__n_estimators": [700, 900, 1200, 1500],
                "model__learning_rate": [0.005, 0.01, 0.015, 0.02, 0.025, 0.03],
                "model__max_depth": [2, 3],
                "model__min_child_weight": [8, 10, 12, 15, 20],
                "model__subsample": [0.60, 0.65, 0.70, 0.75, 0.80],
                "model__colsample_bytree": [0.50, 0.60, 0.70, 0.80],
                "model__gamma": [0.0, 0.05, 0.10, 0.20, 0.30],
                "model__reg_alpha": [0.3, 0.5, 1.0, 2.0, 4.0],
                "model__reg_lambda": [10, 20, 30, 50, 80],
    ...
    if HAS_CATBOOST and feature_set in ["StructGradDesign_CleanCategorical", "FullExpanded_CleanCategorical"]:
        models["CatBoost"] = {
            "estimator": make_catboost(True), "n_iter": N_ITER_OTHER,
            "params": {
...
def main():
    ...
    for fs_name in FEATURE_SETS_TO_RUN:
        ...
        for model_name, spec in define_models(fs_name).items():
            for weighted in ([False, True] if (model_name == "XGBoost" and fs_name in
    ...
    # ---- Select final model (validation/robust score; NEVER the test) ----
    selected = results_df.iloc[0]
    selected_label = selected["Label"]
    obj = trained_objects[selected_label]
    print("\n" + "=" * 100)
    print("FINAL SELECTED MODEL (by robust score; locked test NOT used for selection)")
    print("=" * 100)
    ...
    settings_df = pd.DataFrame([
        ...
        {"Setting": "Split", "Value": "70% train / 10% validation / 20% LOCKED test"},
